...
pub fn decode_standard<T: From<Vec<u8>>>(s: impl AsRef<str>) -> Result<T, DecodeError> {
    let bytes: Vec<u8> = BASE64_STANDARD.decode(s.as_ref())?;
    Ok(T::from(bytes))
}
pub fn decode_urlsafe<T: From<Vec<u8>>>(s: impl AsRef<str>) -> Result<T, DecodeError> {
...
impl<T: From<Vec<u8>>> serde::de::Visitor<'_> for Base64Visitor<T> {
    type Value = Base64<T>;
